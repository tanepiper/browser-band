    ...
    function (error, dirPath, dirs, files) {
      ...
      var patchFiles = _(files)
        ...
        if (patchFiles.length > 0) {
          server.app.patchFiles = server.app.patchFiles.concat(patchFiles);
        }
      }
    );

  server.app.websocket.on('connection', function (socket) {
    socket.on('get-patch-list', function () {
      socket.emit('patch-list', server.app.patchFiles);
    });
  });
